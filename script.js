    ...
    products.forEach(product => {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-content">
                ...
                <p>৳ ${product.price}</p>
                <a href="${product.link}" class="buy-button" target="_blank">Buy</a>
...
function showProductModal(product) {
    ...
    const modalImagesContainer = modal.querySelector('.product-images');
...
// Function to display products with limited text and See More button
function displayProducts() {
    const productsContainer = document.querySelector('.products');
    const products = JSON.parse(localStorage.getItem('products')) || [];
    ...
    products.forEach(product => {
        ...
        const shortDesc = product.description.length > 100 ? 
            product.description.slice(0, 100) + '... <span class="more-button" data-id="' + product.id + '">See More</span>' : 
            product.description;
        ...
        productCard.innerHTML = `
            ...
            <div class="product-content">
                ...
                <p>৳ ${product.price}</p>
                <a href="${product.link}" class="buy-button" target="_blank">Buy</a>
                ${isAdmin ? `<button class="edit-button" data-id="${product.id}">Edit</button><button class="delete-button" data-id="${product.id}">Delete</button>` : ''}
            </div>
        `;
        productsContainer.appendChild(productCard);

        // Add click event to the entire card, excluding the buttons
        productCard.addEventListener('click', function(event) {
            if (!event.target.closest('button') && !event.target.classList.contains('more-button')) {
                showProductModal(product);
            }
        });

        // Add click event to the edit button
        productCard.querySelector('.edit-button')?.addEventListener('click', function(event) {
            event.stopPropagation();
            editProduct(product.id);
        });

        // Add click event to the delete button
        productCard.querySelector('.delete-button')?.addEventListener('click', function(event) {
            event.stopPropagation();
